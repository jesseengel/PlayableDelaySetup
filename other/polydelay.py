...
class polydelay:
    ...
    def __init__(self, send_port, receive_port):
        ...
        self.unit = [0] * self.voices
        self.note = 0
        self.transpose = [0] * self.voices
        self.root_note = 0
        self.root_key = 'C'
        self._attr_list = ['onoffs', 'input_volume', 'output_volume', 'pan', 'feedback', 
                              'time', 'unit']
    ...
    #             self.sendOSC(self.send_port, '/polydelay/pattr', [pattr_var, ival])
    ...
    def set(self, data):
        ...
        if attribute is 'input_volume' or 'output_volume' or 'pan' or 'feedback' \
           or 'time' or 'unit':
            setattr(self, attribute, data)
            # self.send_to_pattr(getattr(self, attribute))
            # send update to launchpad
            self.polydelay_refresh()
            self.launchpad_refresh()
            
        elif attribute is 'pitch' or 'octave':
            setattr(self, attribute, data)
            self.polydelay_refresh()
            self.launchpad_refresh()
            self.calc_transpose()
            
        elif attribute is 'note':
    ...
    def calc_transpose(self): 
        for i,ipitch in enumerate(self.pitch):
            self.transpose[i] = self.transpose_lookup[self.note][ipitch] \
                                + self.octave[i]*12
        self.sendOSC(self.send_port, '/polydelay', ['transpose', self.transpose])
